...
EVALUATION_ROOT = MATERIAL_ROOT / "data" / "response_time" / "main"
...
def relative_source_path(path: Path) -> str:
    """Return a portable path relative to the main response-time data."""
...
def scan_records(results_dir: Path) -> tuple[list[dict], list[dict]]:
    ...
    if not results_dir.is_relative_to(evaluation_root):
        raise RuntimeError(
            f"Plot data must stay inside the main response-time data: {results_dir}"
        )
    records = []
    issues = []
