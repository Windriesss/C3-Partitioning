...
the complete 1,000 Hz runs in the bundled RK3588 response-time data. For every
...
MAIN_RESULTS_DIR = MATERIAL_ROOT / "data" / "response_time" / "main" / "rk3588" / "results"
OUTPUT_DIR = MATERIAL_ROOT / "figures" / "main"
OUTPUT_BASENAME = "rk3588_1000hz_ablation_sensitivity_tresp_1x4"
OUTPUT_BASENAME_1000HZ = "rk3588_1000hz_ablation_sensitivity_tresp_1x4"
FREQUENCIES_HZ = (1000,)
...
def plot_1000hz(stats: list[dict], vm_labels: list[str]) -> tuple[Path, Path]:
    """Create the compact 1000 Hz 1x4 figure used in the paper."""
    frequency_hz = 1000
    ...
    png_path = OUTPUT_DIR / f"{OUTPUT_BASENAME_1000HZ}.png"
    pdf_path = OUTPUT_DIR / f"{OUTPUT_BASENAME_1000HZ}.pdf"
    fig.savefig(png_path, dpi=300)
    fig.savefig(pdf_path)
    plt.close(fig)
    return png_path, pdf_path
...
def main() -> int:
    ...
    if not stats:
        raise RuntimeError(
            "No complete RK3588 1000/100/10 Hz ablation records were found"
        )

    vm_labels = base.available_vm_labels(stats)
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    values_path = save_values(stats, vm_labels)
    png_1000hz_path, pdf_1000hz_path = plot_1000hz(stats, vm_labels)
    print(
    ...
    print(f"[Saved] {png_1000hz_path}")
    print(f"[Saved] {pdf_1000hz_path}")
    return 0
