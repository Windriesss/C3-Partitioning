"""Plot 100-Hz and 10-Hz J_resp from complete single-run observations.
...
def collect_cells(frequency_hz: int) -> tuple[dict[str, list[dict]], list[str]]:
    ...
    for platform, spec in baseline.PLATFORMS.items():
        ...
        allowed_conditions = {
            baseline.condition_name(
                spec["conditions"][method], scenario, frequency_hz
            )
        ...
        records = [
            ...
            if record["condition"] in allowed_conditions
        ]
...
def plot_panel(
    ...
    for method in baseline.METHOD_ORDER:
        ...
        condition = baseline.condition_name(
            condition_map[method], scenario, frequency_hz
        )
...
def save_csv(
    ...
    output = OUTPUT_DIR / f"{output_stem}_single_run_data.csv"
    fields = [
        "platform",
        "scenario",
        "working_set",
                    ...
                    rows.append(
                        {
                            ...
                            "interval_status": (
                                ...
                                if has_ci
                                else f"not shown (single run; n<{TARGET_RUNS})"
                            ),
                        }
                    )
...
def plot_frequency(frequency_hz: int) -> list[Path]:
    output_stem = f"rk3588_rk3568_jresp_{frequency_hz}Hz_2x2"
    ...
    cells_by_platform, vm_labels = collect_cells(frequency_hz)
    if not vm_labels:
        raise RuntimeError(
            f"No complete {frequency_hz}-Hz J_resp records were found"
        )
    ...
    csv_path = save_csv(cells_by_platform, vm_labels, frequency_hz, output_stem)
    print(f"[Saved] {png}")
    print(f"[Saved] {pdf}")
    print(f"[Saved] {csv_path}")
    run_counts = {
        cell["n"]
        for cells in cells_by_platform.values()
        for cell in cells
    }
    if run_counts == {1}:
        print("[Note] Every plotted cell is one complete run; confidence bands omitted.")
    else:
        print(
            f"[Note] 95% bootstrap confidence bands require at least "
            f"{TARGET_RUNS} runs."
        )
